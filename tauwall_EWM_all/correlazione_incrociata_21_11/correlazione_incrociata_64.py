...
dns = np.load('tauwall_reference_1GPU_dns.npz')
ewm = np.load('tauwall_lowres_1GPU_EWM.npz')
u60 = np.load('sample_u60_1GPU.npz')
#model = np.load('tauwall_ML_model_t600_r600.npz')
#model1 = np.load('tauwall_ML_model_t300_r300.npz')
...
#model11 = np.load('tauwall_ML_model_t1000_r125_linear.npz')
model12 = np.load('tauwall_ML_model_t1000_r125_linear.npz')
...
#len_model11 = len(model11.keys())
len_model12 = len(model12.keys())
...
#Rk_model11 = np.zeros((len_model11,k_array.shape[0]))
Rk_model12 = np.zeros((len_model12,k_array.shape[0]))
...
#
#f = 0
#for (k,ku) in zip(model11.keys(), u60.keys()):
#    campo = model11[k]
#    vel = u60[ku]
#    flut_model = campo - np.mean(campo) 
#    flut_vel = vel - np.mean(vel)
#
#    Rk_model11[f,:] = compute_Rk(flut_model, flut_vel, k_array, nx,nz)
#    f = f + 1

f = 0
for (k,ku) in zip(model12.keys(), u60.keys()):
    campo = model12[k]
    vel = u60[ku]
    flut_model = campo - np.mean(campo) 
    flut_vel = vel - np.mean(vel)

    Rk_model12[f,:] = compute_Rk(flut_model, flut_vel, k_array, nx,nz)
    f = f + 1
...
np.save('Rk_ewm.npy',Rk_mean_ewm)
...
#np.save('Rk_DM_t600_r600.npy', Rk_mean_model)
#Rk_mean_model1 = np.mean(Rk_model1, axis=0)
#np.save('Rk_DM_t300_r300.npy', Rk_mean_model1)
#Rk_mean_model2 = np.mean(Rk_model2, axis=0)
#np.save('Rk_DM_t240_r240.npy', Rk_mean_model2)
...
#Rk_mean_model11 = np.mean(Rk_model11, axis=0)
#np.save('Rk_DM_t1000_r125_linear.npy', Rk_mean_model11)
Rk_mean_model12 = np.mean(Rk_model12, axis=0)
np.save('Rk_DM_t820_r102_linear.npy', Rk_mean_model12)
exit()
...
ax.plot(x_dimension, Rk_mean, color='black',label='dns correlation y+=60')
...
ax.plot(x_dimension, Rk_mean_model2, label='DM correlation y+=60 t240 r240')
...
ax.set_xlim(-0.5,0.5)
plt.legend()
plt.savefig('cross_correlation_alpha_central_less.png', dpi=300)
plt.close()
